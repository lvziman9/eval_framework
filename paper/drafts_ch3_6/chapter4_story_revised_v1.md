...
This chapter tests whether validated native-path recommenders exhibit a universal accuracy leader or a common explanation response. The evidence covers the two complete main datasets, LastFM and ML-1M, and the six implementations that pass the Chapter 3 validation gates on both datasets: PGPR, UCPR, CAFE, TPRec, KGGLM, and PEARLM. The chapter reports empirical results rather than revisiting framework design.

Two evidence streams remain separate throughout the analysis. Strict accuracy results come from the per-row accuracy evidence summarised in Table 4.1. Alpha-sweep results come from the canonical trade-off CSVs and describe changes in LIR, SEP, ETD, and their paired sweep ranking metrics as alpha varies. Alpha-sweep ranking values are not used as substitutes for strict accuracy. All explanation results concern paths produced natively within the recommendation workflow; the main comparison contains no post-hoc explanations.
