...
In the repository implementation, \(a(p_{u,i})\) is the seed item at the path position used by the path-metric guide, and \(\rho_u\) is derived from the user's training interaction timestamps. The exact implementation is repository-specific. The function \(\sigma_d\) maps bridge-entity degree information to a normalised SEP weight. Direct inspection of `xrecsys/path_data_loader.py::generate_SEP_matrix()` verifies that the accessible generator sorts by ascending degree and then assigns increasing ordinal EMA weights; the guide's lower-degree / higher-weight wording therefore appears stale for the current code. The historical `sep_matrix.pkl` used for the registered sweeps is not accessible, so its provenance and direction still require manual verification. The formula above consequently records a repository-specific degree-derived bridge-entity lookup without interpreting higher SEP as lower degree, rarity, novelty, usefulness, or user-perceived explanation quality.
...
The explanation metrics add operational limitations. LIR requires valid timestamps and a recoverable interaction anchor. SEP is a repository-specific degree-derived bridge-entity score whose accessible generator direction is verified but whose historical matrix-cache provenance remains unavailable. ETD depends on a declared taxonomy and denominator for legal path types. These definitions are auditable but do not exhaust explanation quality and cannot be collapsed into one latent construct. Their conceptual source is verified through XRecSys, while the exact formulas and data assumptions evaluated here remain repository-specific [@balloccu2022xrecsys].
