# Table and Figure Evidence Refresh Plan: Batch 1C

## 1. Scope

This plan retains the Batch 1B evidence-role refresh for Table 3.6, Table 5.2, Table 5.4, and existing Figures 3.1-5.2; updates SEP wording after the Batch 1C code audit; and registers ten new diagram specifications. It does not regenerate figures, insert images, change experimental values, or perform final NTU, Word, PDF, or Markdown formatting.

The revised standalone text is stored in `REVISED_TABLES_AND_CAPTIONS_BATCH1C.md`. Existing table and figure source files remain unchanged.
...
| Table 5.4 | `paper/drafts_ch3_6/chapter5_tables.md` | Citation and provenance caveats are distributed across several rows; SEP historical cache provenance remains unresolved. | Consolidate user-study, significance, Amazon, metric-implementation, strict-primary-artifact, bibliography, configuration, and historical SEP-cache caveats. | Refreshed in standalone output |
...
3. LIR, SEP, and ETD are computational path properties under repository-specific implementations, not user-perceived explanation quality. SEP is described only as a repository-specific degree-derived bridge-entity score; the historical matrix cache remains subject to manual verification.
...
## 6. Batch 1C Diagram Inventory

| Figure / item | Placement | Main text / appendix | Evidence role | Draft source | Preferred final source | Rendering status |
| --- | --- | --- | --- | --- | --- | --- |
| Framework overview and dataflow | Chapter 3.1 | Main text | Evaluation dataflow | Mermaid + DOT | DOT/SVG | spec only |
| Validation gate flowchart | Chapter 3.4 | Main text | Reportability checks | Mermaid + DOT | DOT/SVG | spec only |
| Metric anchor schematic | Chapter 3.4 | Main text or appendix | LIR/SEP/ETD anchors | Mermaid + DOT | DOT/SVG or draw.io | spec only |
| Single-example trace | Chapter 3.3 | Main text | Artifact alignment example | Markdown table | Word table / SVG | spec only |
| Alpha-sweep process | Chapter 3.6 or 4.1 | Main text | Trade-off setup | Mermaid + DOT | DOT/SVG | spec only |
| Evidence-stream separation | Chapter 3.6 | Main text | Prevents evidence mixing | Mermaid + DOT | DOT/SVG | spec only |
| Trade-off pattern schematic | Chapter 4.7 | Main text | Findings summary | Markdown + DOT | SVG | spec only |
| Ablation evidence flow | Chapter 5.1 | Main text | Ablation protocol | Mermaid + DOT | DOT/SVG | spec only |
| 95% retention operating point | Chapter 5.1 | Main text or appendix | Operating-point rule | Mermaid + DOT | DOT/SVG | spec only |
| Amazon boundary flow | Chapter 5.4 | Main text or appendix | Boundary reportability | Mermaid + DOT | DOT/SVG | spec only |

## 7. Caption Requirements

1. Framework captions state: “This diagram describes the evaluation framework and does not define a new recommender architecture.”
2. Validation and boundary captions state: “This figure records validation status rather than comparative recommendation performance.”
3. The single-example caption states: “The single example illustrates artifact alignment and metric anchors; it is not a standalone experimental result.”
4. Alpha-sweep captions state: “The sweep NDCG in this figure belongs to the alpha-sweep evidence stream and must not be interpreted as strict NDCG@10.”
5. SEP captions use “repository-specific degree-derived bridge-entity score” and retain the historical matrix-cache caveat.

## 8. Deferred Work

Final numbering style, caption typography, image insertion, panel assembly, pagination, cross-references, bibliography generation, and NTU Word/PDF formatting remain deferred. No figure is rendered or regenerated in Batch 1C.
