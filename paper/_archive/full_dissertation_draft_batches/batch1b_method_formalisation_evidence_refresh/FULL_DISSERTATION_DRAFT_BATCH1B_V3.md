...
The function \(a(p_{u,i})\) denotes the linked historical seed item, \(b(p_{u,i})\) denotes the bridge entity, and \(\tau(p_{u,i})\) denotes the path type used for diversity. The corresponding path-level definitions are
...
In the repository implementation, \(a(p_{u,i})\) is the seed item at the path position used by the path-metric guide, and \(\rho_u\) is derived from the user's training interaction timestamps. The exact implementation is repository-specific. The function \(\sigma_d\) maps bridge-entity degree information to a normalised SEP weight. The path-metric guide describes lower-degree bridge entities as receiving higher serendipity weights, but direct inspection of `xrecsys/path_data_loader.py::generate_SEP_matrix()` shows ascending degree order followed by increasing EMA weights. The monotonic direction must therefore be reconciled manually before final submission; the formula above records the implemented degree-to-weight lookup without claiming that the guide and code are already consistent.
...
SEP operationalises serendipity through the normalised rarity of the bridge entity under the repository implementation. Higher SEP should consequently be described as greater exposure to rarity- or serendipity-oriented bridge entities, not automatically as novelty, usefulness, or better explanations for users. The curves do not identify the model mechanism responsible for their different shapes; that interpretation remains in Chapter 5.
...
The registered SEP values and Batch 1 curve findings are retained unchanged. However, the path-metric guide's low-degree / high-weight interpretation and the accessible SEP matrix-generation code are not yet directionally reconciled. Interpretation of higher SEP as greater bridge-entity rarity therefore remains provisional pending manual formula/code verification.
