...
On ML-1M, PGPR, CAFE, and TPRec again reach high alpha=1 SEP values but incur different NDCG changes. UCPR increases SEP from 0.4935 to 0.7406 while NDCG changes only from 0.0862 to 0.0825. KGGLM remains unchanged on both measures, and PEARLM records a small SEP increase with nearly unchanged NDCG. Figure 4.5 retains the complete trajectories. These profiles show that movement along the implemented SEP objective occurs at different paired ranking costs; they do not make SEP a complete explanation-quality measure.

Across the two datasets, SEP often permits strong endpoint movement, but the common direction of that movement does not imply a common ranking cost. PGPR, UCPR, CAFE, and TPRec can all move towards higher SEP while preserving different proportions of paired NDCG, and KGGLM and PEARLM expose more restricted controllability in the registered sweeps. The result-level finding is therefore variation in SEP controllability, not one uniformly favourable trade-off. The SEP-oriented sweep is interpreted as movement along the implemented SEP objective.

The SEP-oriented sweep provides one of the clearest empirical trade-off profiles in the alpha-sweep results. Across the registered alpha grid, several models exhibit substantial movement along the implemented SEP objective, while paired sweep NDCG changes at different rates. This makes SEP particularly useful for observing how strongly a model can be steered by an explanation-side objective under the shared evaluation framework. The SEP curves are useful for analysing framework controllability under an explanation-side objective.

SEP is a repository-specific bridge-entity score following the XRecSys-style explanation-metric setting and is interpreted here through the implemented repository metric pipeline. SEP is used as an operational explanation-side metric rather than as direct evidence of user-perceived serendipity. SEP endpoint movement should not be read as direct evidence that users would perceive the explanations as more serendipitous, surprising, or useful. It also does not establish higher explanation quality. The curves do not identify the model mechanism responsible for their different shapes; that interpretation remains in Chapter 5.
...
The registered SEP values and curve findings are retained unchanged. The dissertation does not independently validate the semantic direction of SEP against user perception or external serendipity judgments. SEP-oriented results describe movement along the implemented SEP objective.
