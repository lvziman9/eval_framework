...
OUTPUT = ROOT / "paper/docx_build_v3/DISSERTATION_NTU_TEMPLATE_DRAFT_V3.docx"
AUDIT = ROOT / "paper/docx_build_v3/reports/ASSEMBLY_AUDIT.json"
...
def extract_template_logo() -> None:
    ...
    with zipfile.ZipFile(TEMPLATE) as archive:
        ...
        if "word/media/image1.jpg" in candidates:
            selected = "word/media/image1.jpg"
        elif candidates:
            selected = candidates[0]
        else:
            raise RuntimeError("NTU template contains no logo image")
        LOGO.write_bytes(archive.read(selected))
...
def add_cover_and_forms(document, template_texts: list[str]) -> None:
    ...
    add_form_page(
        ...
        [template_texts[index] for index in [28, 29, 30, 31]],
        ("Date", "Candidate name and signature"),
    )
    add_form_page(
        document,
...
def add_list_index(document, heading: str, entries: list[str], page_map: dict[str, str]) -> None:
    ...
    for text in entries:
        ...
        entry.add_run(text)
        entry.add_run("\t")
        identifier = re.match(r"^(Figure|Table)\s+((?:\d+\.)+\d+[a-z]?|[A-Z]\.\d+)", text)
...
def set_repeat_table_header(row) -> None:
    ...
    if no_split is not None:
        table_row_properties.remove(no_split)
...
def set_cell_bottom_border(cell, color: str = "000000", size: str = "8") -> None:
    cell_properties = cell._tc.get_or_add_tcPr()
...
def set_table_borders(table) -> None:
    ...
    for edge in ("top", "left", "bottom", "right", "insideH", "insideV"):
        ...
        border.set(qn("w:sz"), "3" if edge == "insideH" else "8")
        border.set(qn("w:space"), "0")
        border.set(qn("w:color"), "B3B3B3" if edge == "insideH" else "000000")
...
def table_requires_landscape(table) -> bool:
    # V3 table content is explicitly simplified to fit the portrait text block.
    return False
...
def format_tables(document) -> list[int]:
    ratio_map = {
        "3.1": [16, 16, 16, 24, 28],
        "3.2": [12, 18, 24, 23, 23],
        "4.1": [12, 12, 19, 19, 19, 19],
        "4.2": [14, 14, 24, 24, 24],
        "5.1": [15, 12, 13, 15, 22, 23],
        "5.3a": [15, 15, 20, 25, 25],
        "5.3b": [15, 20, 40, 25],
    }
    ...
    for table_number, table in enumerate(document.tables, 1):
        ...
        target_width = 8640
        caption_node = table._tbl.getnext()
        while caption_node is not None and caption_node.tag == qn("w:p"):
            caption = Paragraph(caption_node, table._parent)
            if caption.text.strip():
                break
            caption_node = caption_node.getnext()
        else:
            caption = None
        if caption is None or not caption.text.strip().startswith("Table "):
            raise RuntimeError(f"Table {table_number} has no caption immediately below it")
        identifier_match = re.match(r"^Table\s+(\d+\.\d+[a-z]?)\.", caption.text.strip())
        if not identifier_match:
            raise RuntimeError(f"Could not identify Table {table_number}: {caption.text}")
        identifier = identifier_match.group(1)
        ratios = ratio_map.get(identifier)
        if ratios:
            if len(ratios) != len(table.columns):
                raise RuntimeError(f"Width ratio mismatch for Table {identifier}")
            widths = [int(target_width * ratio / sum(ratios)) for ratio in ratios]
            widths[-1] += target_width - sum(widths)
        else:
            widths = allocate_widths(table, target_width)
...
def style_imported_body(document) -> tuple[int, int, int]:
    ...
    for paragraph in document.paragraphs:
        ...
        caption_match = re.match(
            r"^(Figure|Table)\s+((?:\d+(?:\.\d+)*[a-z]?)|(?:[A-Z]\.\d+))\.\s+",
            text,
        )
        if caption_match:
        ...
        if references_active and text and paragraph.style.name in {"Normal", "Bibliography"}:
...
figure_entries = [
    ...
    for element in body_elements
    if re.match(r"^Figure\s+(?:\d+(?:\.\d+)+[a-z]?|[A-Z]\.\d+)\.\s+", direct_child_text(element))
]
table_entries = [
    direct_child_text(element)
    for element in body_elements
    if re.match(r"^Table\s+(?:\d+(?:\.\d+)+[a-z]?|[A-Z]\.\d+)\.\s+", direct_child_text(element))
]
    ...
    json.dumps(
        {
            ...
            "template_used_as_mother_document": True,
            "template_used_as_layout_form_and_asset_source": True,
            "template_sample_body_removed_before_assembly": True,
